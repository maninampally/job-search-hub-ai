...
outreachRoutes.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch outreach logs", details: error.message });
  }
});
...
outreachRoutes.post("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create outreach log", details: error.message });
  }
});
...
outreachRoutes.patch("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to update outreach log", details: error.message });
  }
});
...
outreachRoutes.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to delete outreach log", details: error.message });
  }
});
