...
contactRoutes.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch contacts", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
contactRoutes.post("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create contact", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
contactRoutes.patch("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to update contact", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
contactRoutes.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to delete contact", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
