...
reminderRoutes.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch reminders", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
reminderRoutes.post("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create reminder", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
reminderRoutes.patch("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to update reminder", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
...
reminderRoutes.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to delete reminder", ...(process.env.NODE_ENV !== "production" && { details: error.message }) });
  }
});
