...
async function parseResponse(response) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 402 && body.error === "upgrade_required") {
      try { useUIStore.getState().openModal("upgrade", { feature: body.feature, minTier: body.min_tier }); } catch {}
      message = `This feature requires ${body.min_tier || "Pro"} plan.`;
    }
